# Marketing Code Management for Yourl.Cloud
# ========================================
#
# This script provides comprehensive marketing code management
...
class MarketingCodeManager:
    """Marketing code management class."""
    ...
    def generate_code(self, length: int = 8, prefix: str = "") -> str:
        ...
        while True:
            ...
            code = prefix + ''.join(secrets.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') for _ in range(length))
    ...
    def create_code(self, description: str, discount_percent: float = 0.0, 
                   max_uses: int = -1, valid_days: int = 30, 
                   prefix: str = "", length: int = 8) -> Optional[str]:
        """Create a new marketing code."""
        try:
            code = self.generate_code(length, prefix)
    ...
    def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Expired codes
            self.cursor.execute('''
                SELECT COUNT(*) FROM marketing_codes 
                WHERE valid_until IS NOT NULL AND valid_until < CURRENT_TIMESTAMP
            ''')
            expired_codes = self.cursor.fetchone()[0]
    ...
    def bulk_create_codes(self, count: int, description: str, 
                         discount_percent: float = 0.0, max_uses: int = 1,
                         valid_days: int = 30, prefix: str = "", length: int = 8) -> List[str]:
        """Create multiple marketing codes at once."""
        created_codes = []
        ...
        print(f"✅ Created {len(created_codes)} marketing codes")
...
def main():
    ...
    parser.add_argument('action', choices=['create', 'list', 'info', 'use', 'update', 'delete', 'stats', 'bulk'],
    ...
    parser.add_argument('--prefix', help='Code prefix')
    parser.add_argument('--length', type=int, help='Code length')
    parser.add_argument('--count', type=int, help='Number of codes to create (for bulk)')
    ...
    try:
        ...
        if args.action == 'create':
            ...
            if not args.description or not args.count:
                print("❌ Description and count required for bulk action")
                exit(1)
            
            codes = manager.bulk_create_codes(
                count=args.count,
                description=args.description,
                discount_percent=args.discount or 0.0,
                max_uses=args.max_uses or 1,
            ...
            print(f"📋 Created codes: {', '.join(codes)}")
