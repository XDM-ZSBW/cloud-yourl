# Marketing Code Storage for Yourl.Cloud
# ======================================
#
# This script provides database storage and management for marketing codes
# in Yourl.Cloud applications.
#
...
class MarketingCodeStorage:
    """Marketing code storage and management class."""
    ...
    def ensure_tables_exist(self) -> bool:
        """Ensure marketing codes table exists."""
        try:
            self.cursor.execute('''
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Marketing Code Storage for Yourl.Cloud')
    ...
    storage = MarketingCodeStorage(args.db_path)
    ...
    try:
        ...
        if args.action == 'create':
            ...
            info = storage.get_code_info(args.code)
            if info:
            ...
            result = storage.use_code(args.code)
            print(json.dumps(result, indent=2))
            
        elif args.action == 'stats':
            stats = storage.get_usage_statistics()
            ...
            if updates:
                success = storage.update_code(args.code, **updates)
            ...
            success = storage.delete_code(args.code)
            exit(0 if success else 1)
