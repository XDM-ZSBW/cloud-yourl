...
function Invoke-TestClipboardHistory {
    ...
    # Build the command
    $command = "python scripts/test_clipboard_history_local.py $Action"
...
$scriptPath = "scripts/test_clipboard_history_local.py"
...
# Run the test clipboard history integration
try {
    Invoke-TestClipboardHistory -Action $Action -Query $Query -Tags $Tags -Hours $Hours -Content $Content
}
catch {
    Write-Error "Failed to run test clipboard history integration: $_"
    exit 1
}
